...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic == 'org.cta.table.stations.transformed': # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic == 'org.cta.station.arrivals': # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif message.topic == 'org.cta.station.turnstile': # Set the conditional to the KSQL Turnstile Summary Topic
